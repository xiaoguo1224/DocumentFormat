#!/usr/bin/env python3
"""Profile/template-driven structural validator for Word-native DOCX features."""
...
def _story_has_content(root) -> bool:
    if root is None:
        return False
    if any((t.text or '').strip() for t in root.iter(W + 't')):
        return True
    return any(el.tag in {W + 'drawing', W + 'pict', W + 'fldChar'} for el in root.iter())


def _story_summary(zf) -> dict[str, int]:
    summary = {'headers': 0, 'footers': 0}
    for name in zf.namelist():
        if name.startswith('word/header') and name.endswith('.xml'):
            summary['headers'] += int(_story_has_content(read_xml(zf, name)))
        elif name.startswith('word/footer') and name.endswith('.xml'):
            summary['footers'] += int(_story_has_content(read_xml(zf, name)))
    return summary
...
def validate(
    ...
    require_crossrefs=False,
    require_reference_numbering=False,
):
    ...
    with zipfile.ZipFile(path) as zf:
        ...
        if styles is not None:
            for st in styles.findall(W + 'style'):
                ...
                name_to_id[(name or '').casefold()] = sid
                ppr = st.find(W + 'pPr')
        ...
        for i in range(1, max_level + 1):
            sid = name_to_id.get(style(profile, f'heading_{i}', f'Heading {i}').casefold())
            heading_ids.append(sid)
            bindings.append(info.get(sid))
        heading_ok = all(bindings)
        if heading_ok:
            ids = [x['num_id'] for x in bindings]
            levels = [x['ilvl'] for x in bindings]
        ...
        if numbering is not None and heading_ok:
            num_id = bindings[0]['num_id']
            abs_id = None
            for n in numbering.findall(W + 'num'):
                if val(n, 'numId') == num_id:
                    abs_id = val(n.find(W + 'abstractNumId')); break
            if abs_id is not None:
                for a in numbering.findall(W + 'abstractNum'):
                    if val(a, 'abstractNumId') == abs_id:
                        num_levels_ok = len(a.findall(W + 'lvl')) >= max_level; break
        ...
        if doc is not None:
            ...
            for sect in doc.iter(W + 'sectPr'):
                section_formats.append(val(sect.find(W + 'pgNumType'), 'fmt'))
            for p in doc.iter(W + 'p'):
                ...
                if sid in heading_ids and re.match(r'^\d+(?:\.\d+){0,8}\s+\S', txt):
                    static_heading.append(txt[:80])
                if sid == ref_style_id and txt:
                    ref_paras.append((p, txt))
                if re.match(r'^(图|表|Figure|Table)\s*\d', txt, re.I):
                    caption_candidates.append((txt[:100], field_text(p)))
                style_name = info.get(sid, {}).get('name', '').casefold()
                ...
                if not excluded and ' REF ' not in f' {para_fields} ':
                    ...
                    static_cross.extend(m.group(0) for m in re.finditer(pattern, txt, re.I))
        ...
        blob = '\n'.join(fields).upper()
        toc_ok = bool(re.search(r'(^|\s)TOC(\s|$)', blob))
        seq_ok = bool(re.search(r'(^|\s)SEQ\s', blob))
        page_ok = bool(re.search(r'(^|\s)PAGE(\s|$)', blob))
        ref_targets = re.findall(r'(?:^|\s)REF\s+([A-Z_][A-Z0-9_.]*)', blob, re.I)
        ...
        bookmarks = set()
        if doc is not None:
            bookmarks = {val(n, 'name').upper() for n in doc.iter(W + 'bookmarkStart') if val(n, 'name')}
        ...
        if caption_candidates:
            bad = [x for x in caption_candidates if 'SEQ ' not in (x[1] or '').upper()]
            add('possible static captions', 'WARN' if bad else 'PASS', f'{len(bad)} caption-looking paragraph(s) lack SEQ' if bad else 'caption-looking paragraphs contain SEQ')

        h1_sid = heading_ids[0] if heading_ids else None
        need_pb = bool(get(profile, 'headings', 'use_page_break_before', default=True))
        pb = info.get(h1_sid, {}).get('page_break_before', False)
        ...
        expected_front = get(profile, 'pagination', 'front_matter', 'format', default=None)
        expected_body = get(profile, 'pagination', 'body', 'format', default=None)
        if expected_front and expected_body and expected_front != expected_body and len(section_formats) > 1:
            pagination_ok = expected_front in section_formats and expected_body in section_formats
            add('section page-number formats', 'PASS' if pagination_ok else 'FAIL', f'expected front={expected_front}, body={expected_body}; found={section_formats}')
        elif expected_body and section_formats:
            body_ok = expected_body in section_formats or all(value is None for value in section_formats)
            add('section page-number formats', 'PASS' if body_ok else 'WARN', f'body={expected_body}; found={section_formats}')

        if template_path is not None and template_path.is_file():
            with zipfile.ZipFile(template_path) as zt:
                wanted = _story_summary(zt)
            found = _story_summary(zf)
            story_ok = found['headers'] >= min(1, wanted['headers']) and found['footers'] >= min(1, wanted['footers'])
            add('template header/footer presence', 'PASS' if story_ok else 'FAIL', f'template={wanted}, output={found}')

        ref_native = False
        ref_detail = 'reference style not found'
                ...
                if abs_id:
                    for a in numbering.findall(W + 'abstractNum'):
                        if val(a, 'abstractNumId') == abs_id:
                            ...
                            if lvl is not None:
                                text = val(lvl.find(W + 'lvlText')); suff = val(lvl.find(W + 'suff'))
                                expected = str(get(profile, 'references', 'numbering', 'format', default='[%1]'))
                                exp_suff = str(get(profile, 'references', 'numbering', 'suffix', default='tab'))
                                ref_native = text == expected and suff == exp_suff
                                ref_detail = f'lvlText={text!r}, suffix={suff!r}, numId={num_id}'
        ...
        if doc is not None:
            for p in doc.iter(W + 'p'):
                ft = field_text(p)
                if 'REF _FmtRef' in ft:
                    ref_para_fields.append(ft)
    ...
    return {'file': str(path), 'profile': str(profile.get('_profile_path', '')), 'template': str(template_path or ''), 'overall': overall, 'checks': checks}
...
def main():
    ...
    ap.add_argument('--require-reference-numbering', action='store_true')
    args = ap.parse_args()
    result = validate(args.docx, args.profile, args.template, args.require_toc, args.require_captions, args.require_multilevel, args.require_crossrefs, args.require_reference_numbering)
